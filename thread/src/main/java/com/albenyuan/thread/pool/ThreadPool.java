...
public class ThreadPool {
     ...
     *
     */
    private static final ExecutorService CACHE_EXECUTOR = Executors.newCachedThreadPool();
     ...
     *
     */
    private static final ExecutorService FIXED_EXECUTOR = Executors.newFixedThreadPool(CORE_THREAD);

    /**
     * 调度型线程池,调度型线程池会根据Scheduled(任务列表)进行延迟执行，或者是进行周期性的执行.适用于一些周期性的工作.
     */
    private static final ExecutorService SCHEDULED_EXECUTOR = Executors.newScheduledThreadPool(CORE_THREAD);


    private static final ThreadPoolExecutor POOL = new ThreadPoolExecutor(CORE_THREAD, MAX_THREAD, KEEP_ALIVE, TIME_UNIT, QUEUE);
